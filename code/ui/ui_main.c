...
static void UI_DrawTeamMember(rectDef_t *rect, float scale, vec4_t color, qboolean blue, int num, int textStyle) {
	...
	if (value <= 0) {
		...
		if (ui_actualNetGameType.integer == GT_TEAMSURVIVOR) {
...
static qboolean UI_OwnerDrawVisible(int flags) {
	...
	while (flags) {
		...
		if (flags & UI_SHOW_ANYTEAMGAME) {
			if (uiInfo.gameTypes[ui_gameType.integer].gtEnum == GT_TEAMSURVIVOR ) {
				vis = qfalse;
			}
		...
		if (flags & UI_SHOW_NETANYTEAMGAME) {
			if (uiInfo.gameTypes[ui_netGameType.integer].gtEnum == GT_TEAMSURVIVOR ) {
				vis = qfalse;
			}
	...
	if (String_Parse(args, &name)) {
		if (Q_stricmp(name, "StartServer") == 0) {
		  ...
		  trap_Cvar_Set( "cl_paused", "1" );
			...
			if (trap_Cvar_VariableValue("g_gametype") == GT_TEAMSURVIVOR) {
