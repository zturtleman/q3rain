...
static qboolean CG_RainParticleCheckVisible(cg_atmosphericParticle_t *particle) {
    ...
    Com_Printf("^1CG_RainParticleCheckVisible\n");
    return;
...
static qboolean CG_RainParticleGenerate(cg_atmosphericParticle_t *particle, vec3_t currvec, float currweight) {
    ...
    Com_Printf("^1CG_RainParticleGenerate\n");
    return;
...
static void CG_RainParticleRender(cg_atmosphericParticle_t *particle) {
    ...
    Com_Printf("^1CG_RainParticleRender\n");
    return;
...
void CG_EffectParse(const char *effectstr) {
    ...
    Com_Printf("^2CG_EffectParse called with str = '%s'\n", effectstr);

    if (CG_AtmosphericKludge()) {
        Com_Printf("^1CG_AtmospericKludge() == qtrue\n", effectstr);
        return;
    }
        ...
        *eqptr++ = 0;
        ...
        if (!type) {
            if (Q_stricmp(startptr, "T")) {
                cg_atmFx.numDrops = 0;
                Com_Printf("^1Atmospheric effect must start with a type.\n");
                return;
            }
            if (!Q_stricmp(eqptr, "RAIN")) {
                type = "rain";
                Com_Printf("^2using rain\n");
                ...
                cg_atmFx.numDrops = 0;
                Com_Printf("^1Only effect type 'rain' and 'snow' are supported.\n");
                return;
            }
        } else {
            ...
            else Com_Printf("^1Unknown effect key '%s'.\n", startptr);
        }
        startptr = endptr;
    }
...
void CG_AddAtmosphericEffects() {
    ...
    //Com_Printf("^2CG_AddAtmosphericEffects() called\n");
