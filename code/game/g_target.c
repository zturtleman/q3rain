...
================================================================================
================================================================================
============================= RAIN TARGET ENTITIES =============================
================================================================================
================================================================================
 */

/*QUAKED target_win (0.3 0.1 0.6) (-8 -8 -8) (8 8 8)
 Finishes the game and lets activating team win the map. Also sets the next map.

"targetname"            ya should know that
"nextmap"		mapname, overwrites g_nextmap cvar
...
void condition_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
    ...
    if (self->has_weapon >= 0) {
        ...
        if (activator->client->ps.stats[STAT_WEAPONS] & (1 << self->has_weapon)) {
    ...
    if (self->has_powerup >= 0) {
        if (!activator->client) {
            return;
        }
        if (activator->client->ps.powerups[self->has_powerup] > level.time) {
            if (self->invert != qtrue) {
                condition_triggertargets(self, activator);
            }
        } else {
    ...
    if (self->is_walking != 0) {
        ...
        if (activator->client->pers.cmd.buttons & BUTTON_WALKING || activator->client->pers.cmd.upmove < 0) {
            if (self->invert != qtrue) {
                condition_triggertargets(self, activator);
            }
        } else {
    ...
    if (self->active_weapon >= 0) {
        if (self->active_weapon > MAX_WEAPONS) {
            G_Printf("^3WARNING: Entity %s: active_weapon > MAX_WEAPONS, will never trigger\n", self->targetname);
        }
        if (!activator->client) {
            return;
        }
...
void SP_target_condition(gentity_t *self) {
    ...
    G_SpawnInt("standalone", "0", &i);
    if (i != 0) {
        self->standalone = qtrue;
        if (self->leveltime_equal > 0) {
            self->think = condition_firetarget;
            self->nextthink = self->leveltime_equal;
        }
    } else {
        self->standalone = qfalse;
    }

    G_SpawnInt("invert", "0", &i); // done
    if (i != 0) {
        self->invert = qtrue;
    } else {
        self->invert = qfalse;
    ...
    G_SpawnInt("has_weapon", "-1", &self->has_weapon); // done
