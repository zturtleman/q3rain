...
void G_InitGame(int levelTime, int randomSeed, int restart) {
    ...
    level.roundState = ROUND_SPAWNING;
...
void FindIntermissionPoint(void) {
    ...
    if (!ent) { // the map creator forgot to put in an intermission point...
        SelectSpawnPoint(vec3_origin, level.intermission_origin, level.intermission_angle, qfalse, qfalse, qfalse, qfalse);
    } else {
...
void CheckExitRules(void) {
    ...
    if (level.nextState && level.nextStateTime <= level.time) {
    ...
    if (level.roundState != ROUND_PROGRESS) {
        return;
    }
    ...
    if (g_gametype.integer == GT_TEAMSURVIVOR) {
        if (MembersAlive(TEAM_BLUE) <= 0) {
            ...
            RoundEnd();
        } else if (MembersAlive(TEAM_RED) <= 0) {
            trap_SendServerCommand(-1, "cp \"Blue ^7wins the round\n\"");
            Com_Printf("blue wins the round\n");
            level.teamScores[TEAM_BLUE]++;
            RoundEnd();
        }
        if (g_fraglimit.integer) {
            if (level.teamScores[TEAM_RED] >= g_fraglimit.integer) {
                trap_SendServerCommand(-1, "cp \"^1Red ^7hit the fraglimit.\n\"");
                LogExit("Fraglimit hit.");
                return;
            }
            
            if (level.teamScores[TEAM_BLUE] >= g_fraglimit.integer) {
                trap_SendServerCommand(-1, "cp \"Blue ^7hit the fraglimit.\n\"");
                LogExit("Fraglimit hit.");
                return;
            }
