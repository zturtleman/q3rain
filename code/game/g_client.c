 ...
 * ============
 */
gentity_t *SelectRandomFurthestSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles, qboolean isbot, qboolean istarget, qboolean ispolice, qboolean iscivil) {
  gentity_t *spot;
  vec3_t delta;
  float dist;
...
gentity_t *SelectSpawnPoint(vec3_t avoidPoint, vec3_t origin, vec3_t angles, qboolean isbot, qboolean istarget, qboolean ispolice, qboolean iscivil) {
  return SelectRandomFurthestSpawnPoint(avoidPoint, origin, angles, isbot, istarget, ispolice, iscivil);
 ...
 * ============
 */
gentity_t *SelectInitialSpawnPoint(vec3_t origin, vec3_t angles, qboolean isbot, qboolean istarget, qboolean ispolice, qboolean iscivil) {
  gentity_t *spot;
...
void BodySink(gentity_t *ent) {
  ...
  //ent->s.pos.trBase[2] -= 1; //dont sink away
}
...
void ClientUserinfoChanged(int clientNum) {
  ...
  if (g_gametype.integer >= GT_ASSASSINS && g_entities[clientNum].r.svFlags & SVF_BOT) {
...
char *ClientConnect(int clientNum, qboolean firstTime, qboolean isBot) {
  ...
  // don't do the "x connected" messages if they were caried over from previous level
  if (firstTime) {
...
void ClientBegin(int clientNum) {
  ...
  if (client->sess.sessionTeam != TEAM_SPECTATOR) {
    ...
    if (g_gametype.integer == GT_FFA) {
      ...
      trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE " joined the mission.\n\"", client->pers.netname));
    } else if (g_gametype.integer == GT_TEAMSURVIVOR) {
      trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE " joined the fight.\n\"", client->pers.netname));
    } else if (g_gametype.integer == GT_SINGLE_PLAYER) {
      trap_SendServerCommand(-1, va("print \"%s" S_COLOR_WHITE " joined.\n\"", client->pers.netname));
    }
  }
...
void ClientSpawn(gentity_t *ent) {
  ...
  client->ps.weaponPitch = 0;
  client->ps.pitchRate = 1;
